...
fn sphere_fall_into_plane() {
    ...
    let sphere_1 = GameObject::Sphere_default(10.0, [0, 0, 0], [1.0, 2.0, 20.], 10., 0.2, 0.01);
    renderer.add_obj(&sphere_1).unwrap();
    scene.add(sphere_1);

    let mut sphere_2 =
        GameObject::Sphere_default(10.0, [255, 0, 0], [1.0, 25.0, 20.], 10., 0.2, 0.01);
    sphere_2.add_velocity([0.0, -1.0, 0.0]);
    renderer.add_obj(&sphere_2).unwrap();
    scene.add(sphere_2);
